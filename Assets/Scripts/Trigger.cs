...
public class Trigger : MonoBehaviour
{
    ...
    private IEnumerator Switch(float duration)
    {
        ...
        while(currentTime < duration)
        {
            ...
            _switch.localPosition = new Vector3(_switch.position.x, yPosition, _switch.position.z);

            yield return null;
        }

        _switch.localPosition = new Vector3(_switch.position.x, endPoint, _switch.position.z);
